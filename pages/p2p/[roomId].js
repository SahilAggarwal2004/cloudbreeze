...
export default function Id({ router }) {
    ...
    useEffect(() => {
        ...
        peer.on('open', () => {
            let fileName, fileSize, sendProgress, bytes, totalBytes = 0, blob = new Blob([]);
            ...
            conn.on('data', ({ type, name, length, totalSize, file, size, initial = false, progress }) => {
                if (type === 'details') {
                    ...
                    if (sendProgress) conn.send({ type: 'progress', bytes, totalBytes })
