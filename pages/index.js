...
export default function Home() {
    ...
    return <div className='flex flex-col mb-12 space-y-8 md:flex-row md:space-y-0 items-center justify-evenly text-center md:h-[calc(100vh-8rem)]'>
        <div className='shadow-md rounded-lg w-80 h-80 max-w-[80vw] cursor-pointer hover:shadow-2xl transition-all duration-300 relative' onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onClick={() => { setHover(true); redirect('/file/upload') }} onContextMenu={event => event.preventDefault()}>
        ...
        <div className='shadow-md rounded-lg w-80 h-80 max-w-[80vw] cursor-pointer hover:shadow-2xl transition-all duration-300 relative' onClick={() => redirect('/about')} onContextMenu={event => event.preventDefault()}>
