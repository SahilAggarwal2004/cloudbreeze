    ...
    <>
      ...
      <ul className="flex overflow-x-scroll text-xs xs:text-sm font-medium text-center text-gray-500 border-b border-gray-400 px-1">
        {Object.entries(options).map(([option, label]) => (
          <Link key={option} href={`/account/history?filter=${option}`} replace className={`inline-block p-3 rounded-t-lg ${filter === option ? "text-white bg-black cursor-default" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50 cursor-pointer"}`}>
            {label}
            ...
            <table className="min-w-full">
              ...
              <tbody className="cursor-pointer">
                {history.map(({ nameList, name, _id: fileId, createdAt, daysLimit, downloadCount }, i) => {
                  ...
                  return (
                    <tr key={fileId} className="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100" onClick={() => setModal({ active: true, type: "showFile", fileId, filter, downloadCount })}>
